...
#####
# Build a CNN with multiple inputs
#####
...
##################### Load in image files
import os
folder_content=os.listdir(r'D:\Research\Data\DISP\VDF_images_helios_3comps')
#folder_content.sort()
...
for i in range(0,RR):    
  ...
  print(str(i))
lin_images = np.array(all_images)   
lin_images = lin_images.reshape(-1, 100, 100, 1)
...
for i in range(0,RR):    
  ...
  all_images.append(img)
  print(str(i))
log_images = np.array(all_images)   
log_images = log_images.reshape(-1, 100, 100, 1)
...
Xlin_train=lin_images[idx_train,:,:,:]
Xlin_test=lin_images[idx_test,:,:,:]

Xlog_train=log_images[idx_train,:,:,:]
Xlog_test=log_images[idx_test,:,:,:]
...
from cnn_classification_multiple import cnn_classification_multiple
m=cnn_classification_multiple()

# #from gen_AlexNet import gen_AlexNet
# #m=gen_AlexNet()

m.fit([Xlin_train,Xlog_train], y_train, validation_data=([Xlin_test,Xlog_test],y_test), batch_size=100,epochs=30, verbose=1)

pred_labels=np.round(m.predict([Xlin_train,Xlog_train]))
pred_score=(m.predict([Xlin_train,Xlog_test]))
...
# %%
### grid search, 10k sample size should be used
import pickle
from cnn_classification_multiple_grid_search import cnn_classification_multiple_grid_search
...
for k in range(0,len(op_type)):
  ...
  for i in range(0,len(lr)):
    for j in range(0,len(dc)):
      m=cnn_classification_multiple_grid_search(lr[i], dc[j],op_type[k])
      seq_model=m.fit([Xlin_train,Xlog_train], y_train, validation_data=([Xlin_test,Xlog_test],y_test), batch_size=100,epochs=5, verbose=1)
  ...
  filename=(r'D:\Research\Data\Van_Allen_Probes\grid_search_multiple\\' + op_type[k] + '.sav')
